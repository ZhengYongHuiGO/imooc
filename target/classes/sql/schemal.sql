...
CREATE TABLE kill_goods(
  `id` INT AUTO_INCREMENT UNIQUE NOT NULL COMMENT '秒杀商品表',
...
CREATE TABLE success_kill_record(
  `phone` BIGINT  NOT NULL COMMENT '用户的电话（id）',
  `goods_id` INT  NOT NULL COMMENT '商品id',
  `state` INT  DEFAULT -1 NOT NULL COMMENT '-1无效 0未支付 1已支付 2未发货 3 已发货',
  `kill_time` TIMESTAMP  COMMENT '秒杀成功的时间',
